...
In FHIR core, any security label that applies to a resource is recorded at the resource level on `meta.security`. This mechanism works well for cases where either the security label applies to the entire resource, or tracking the precise part of the resource to which the label applies is not important.
